...
tagSize = convertBytesToID3Size(tagSize); %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%tagSize
bytesRead = 0;
...
while bytesRead < tagSize;
    ...
    frameId = char(fread(fid,4,'uint8')'); %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%ID
    ...
    if double(frameId(1)) == 0
        ...
        bytesRead = bytesRead + 4;
        frameSize = convertBytesToID3Size(frameSize); %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%SIZE
        ...
        if frameSize<tagSize
            frameData = fread(fid,frameSize,'uint8')'; %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%DATA
    ...
    if strncmp(frameId,'T',1)
        ...
        switch frameId
            ...
            case 'TIT2'
				...
				frames = printFunc(frames,frameName,frameData);
...
function frames = printFunc(frames,frameName,frameData)
	if frameData(1)==255&&frameData(2)==254
		frames = setfield(frames,frameName,native2unicode(frameData,'UTF-16'));
	else
		frames = setfield(frames,frameName,native2unicode(frameData,'big5'));
	end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%has done Artist Title Album
