...
path = fileparts(which('mp3read'));		% 抓取mp3read這個function所在的路徑；eg: D:\lab_work\audio fingerprinting(matlab)\fingerprint
										%[pathstr,name, ext] = fileparts('')     將''裡面的擋拆成路徑(pathstr)、檔名(name)、副檔名格式(ext)
										%eg.  [pathstr, name, ext] = fileparts('D:\lab_work\music\BadRomance.mp3')
										% pathstr = 'D:\lab_work\music'    name = 'BadRomance'   ext = '.mp3'


% %%%%% Directory for temporary file (if needed)
% % Try to read from environment, or use /tmp if it exists, or use CWD
tmpdir = getenv('TMPDIR');	% getenv: Get environment variable
...
ext = lower(computer);	%lower: from uppercase to lowercase;    COMPUTER returns string C denoting the type of computer on which MATLAB is executing.
if ispc
  ext = 'exe';
  rmcmd = 'del';
end
...
MPG123059 = 0;
mpg123 = fullfile(path,['mpg123.',ext]);	%在目錄底下為path\mpg123.exe 
mp3info = fullfile(path,['mp3info.',ext]);	%在目錄底下為path\mp3info.exe
...
if nargin < 2
  N = 0;	%read just the first N sample frames(scalar)或是two-element vecotr(第幾個frame到第幾個frame)
end

% Check for FMT spec (per wavread)
FMT = 'double';		% FMT as 'native' returns int16 samples instead of doubles; FMT can be 'double' for default behavior (to exactly mirror the syntax of wavread).
if ischar(N)	% vector:0; scalar:0
  FMT = lower(N);	
  N = 0;
end

if length(N) == 1	%vector:2; scalar:1
  % Specified N was upper limit
  N = [1 N];	%若為scalar,表示從第一個frame開始
end
if nargin < 3
  forcemono = 0;
else
  % Check for 3rd arg as FMT
  if ischar(MONO)	% 若輸入的MONO參數為('native', 'double')
    FMT = lower(MONO);	% 換成小寫
    MONO = 0;
  end
  forcemono = (MONO ~= 0);	%判斷MONO是不是不等於0; MONO=0 -> forcemono = 0;  MONO不等於0 -> focemono=1
end
if nargin < 4
  downsamp = 1;
else		% DOWNSAMP = 2 or 4 downsamples by a factor of 2 or 4 (thus FS returns as 22050 or 11025 respectively for a 44 kHz mp3 file);
  downsamp = DOWNSAMP;
end
...
if ~OVERNET	% 歌曲在該目錄底下呼叫mp3info.exe
  %%%%%% Probe file to find format, size, etc. using "mp3info" utility
  cmd = ['"',mp3info, '" -r m -p "%Q %u %b %r %v * %C %e %E %L %O %o %p" "', FILE,'"'];	%產生cmd指令
  ...
  w = mysystem(cmd);	% local funciton:mysystem
						% eg. cmd = "mp3info.exe" -r m -p "%Q %u %b %r %v * %C %e %E %L %O %o %p" "Full Circle.mp3"
						% w = 44100 7975 0 224 1.000000 * No none No III Yes joint stereo No
						% eg. w = 44100(samprate) 7975(#frames) 0(#badframes) 224(bitrate) 1.000000(mpeg version) * No(Copyright) none(emph) No(CRC) III(Layer) Yes(orig) joint(channel mode) stereo(pad) No
  % Break into numerical and ascii parts by finding the delimiter we put in
  starpos = findstr(w,'*');	% starpos = 27
  nums = str2num(w(1:(starpos - 2)));	%nums = 44100        7975           0         224           1
  strs = tokenize(w((starpos+2):end));	%  Columns 1 through 8  'No'    'none'    'No'    'III'    'Yes'    'joint'    'stereo'   'No'
  SR = nums(1);	% 44100(samprate)
  nframes = nums(2);	% 7975 (#frames)
  ...
  elseif SR < 32000 && layer ==3
	...
	% OPTS.fmt is a format info string; OPTS has multiple other fields
...
% process or set delay	% DELAY controls how many "warm up" samples to drop at the start of the file;
if nargin < 5		% 若沒有設置DELAY參數
...
if strcmp(FMT,'size') == 1	%size returns the size of the audio data contained in the file in place of the actual audio data, returning the 2-element vector SIZ=[samples channels].
   Y = [floor(smpspfrm*nframes/downsamp)-delay, nchans];	%  siz = mp3read('Full Circle', 'size'); Y=siz
  ...
  % cmd = "D:\lab_work\audio fingerprinting(matlab)\fingerprint\mpg123.exe" -k 0 -n 11 -q -w "tmp965.wav"  "Full Circle.mp3"
  %w = 
  mysystem(cmd);
...
[s,w] = system(cmd);	% 呼叫system執行cmd指令,擷取音檔資訊放入w並回傳
if s ~= 0 
