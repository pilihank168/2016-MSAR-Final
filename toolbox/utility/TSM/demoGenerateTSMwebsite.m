...
for s = stretchingFactors
    ...
    for i = 1: length(nameFiles)
        ...
        % Original
        [x,fsAudio] = wavread([pathData name]);
        wavwrite(x,fsAudio,[pathHtml name_ORIG]);
        ...
        wavwrite(yOLA,fsAudio,[pathHtml name_OLA]);
        
        % WSOLA
        yWSOLA = wsolaTSM(x,s);
        wavwrite(yWSOLA,fsAudio,[pathHtml name_WSOLA]);
        
        % Phase Vocoder
        yPV = pvTSM(x,s);
        wavwrite(yPV,fsAudio,[pathHtml name_PV]);
        
        % Phase Vocoder with identity phase locking
        paramPVpl.phaseLocking = 1;
        yPVpl = pvTSM(x,s,paramPVpl);
        wavwrite(yPVpl,fsAudio,[pathHtml name_PVpl]);
        
        % TSM based on HPSS
        yHP = hpTSM(x,s);
        wavwrite(yHP,fsAudio,[pathHtml name_HP]);
        
        % elastique
        yELAST = elastiqueTSM(x,s);
        wavwrite(yELAST,fsAudio,[pathHtml name_ELAST]);
